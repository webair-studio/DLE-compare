...
function msgbox($title, $text) {
    global $tpl, $lang;
    if( strpos($text,'для Вас не доступна: возможно, был изменен её адрес или она была удалена. Пожалуйста, воспользуйтесь поиском')|| $text == $lang['news_err_27'] ) {
        require_once ROOT_DIR.'/e404.html';
        die();
...
function build_js($js, $config) {
	...
	if ($config['js_min'] AND version_compare(PHP_VERSION, '5.1.0', '>') ) {
		...
		$js_array[] = "<script type=\"text/javascript\" src=\"{$config['http_home_url']}engine/classes/min/index.php?charset={$config['charset']}&amp;g=general&amp;17\"></script>";

		if ( count($js) ) $js_array[] = "<script type=\"text/javascript\" src=\"{$config['http_home_url']}engine/classes/min/index.php?charset={$config['charset']}&amp;f=".implode(",", $js)."&amp;17\"></script>";
