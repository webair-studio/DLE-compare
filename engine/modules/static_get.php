...
$tpl->clear();
		...
		if( $static_result['allow_template'] or $view_template == "print" ) {
            ...
            if($static_result['tpl'] == "condition") {
                $tpl->set('{current_page}', $_SERVER['REQUEST_URI']);
                $static_result_condition = $db->super_query( "SELECT * FROM " . PREFIX . "_static_condition WHERE static_id = " . $static_result['id']);
                foreach($static_result_condition as $key => $value) {
                    if($key == "condition_preim_1" ||
                        $key == "condition_preim_2" ||
                        $key == "condition_preim_3" ||
                        $key == "condition_preim_4" ||
                        $key == "condition_preim_5" ||
                        $key == "condition_preim_6" ||
                        $key == "condition_preim_7" ||
                        $key == "condition_preim_8" ||
                        $key == "condition_preim_9" ||
                        $key == "condition_preim_10" ||
                        $key == "condition_polomki_1" ||
                        $key == "condition_polomki_2" ||
                        $key == "condition_polomki_3" ||
                        $key == "condition_polomki_4" ||
                        $key == "condition_polomki_5" ||
                        $key == "condition_polomki_6" ||
                        $key == "condition_polomki_7" ||
                        $key == "condition_polomki_8" ||
                        $key == "condition_polomki_9" ||
                        $key == "condition_polomki_10") {
                            $tpl->set( '{'.$key.'}', ($value) ? "<li><b>" . $value . "</b></li>" : "" );
                    }
                    elseif($key == "condition_under_polomki_1") {
                        $tpl->set( '{'.$key.'}', strip_tags($value));
                    }
                    else {
                        $tpl->set( '{'.$key.'}', $value );
                    }
                ...
                for($i = 1; $i <= 18; $i++) {
                    if($static_result_condition['condition_block_name_'.$i] != '') {
                        //echo $i;
                        if($i%2 == 1) {
                            $table .= "<table class='bottom_blocks".(($i > 6)?"  notShowed":"")."'><tbody>";
                        }
                        $table .= "<tr><td><div style='opacity: 1 !important;padding-bottom:7px'><span style=\"font-weight: normal; font-size:16px; opacity: 1; color:#000000;\">".$static_result_condition['condition_block_name_'.$i]."</span></div>".$static_result_condition['condition_block_text_'.$i]."</td></tr>";
                        if($i%2 == 0) {
                            $table .= "</tbody></table>";
                        }
                    }
                }
                $tpl->set( '{condition_blocks}', $table );
            }
			...
			//$template = str_replace("img", "", $template);
			preg_match_all("#<a href.{1,100} title=\"(.{1,300})\".{1,300}>(<img src.{1,400}alt=\".{1,300}\".{1,100}>)</a>#Uis", $template, $image_list);
			$template = str_replace($image_list[0][0], str_replace("/>", " title=\"".$image_list[1][0]."\" />", $image_list[2][0]), $template);
			...
			preg_match_all("/img/", $template, $asdtest);
			//echo "<!--".print_r($asdtest,true)."-->";
			if(count($asdtest[0]) == 1) {
				$template = preg_replace("/(<img.{1,600}>)/", "\\1<h1 style=\"margin-top:15px;margin-bottom:10px\">".$static_descr."</h1>", $template);
				$tpl->set( '{description}', "" );
			}
			else {
				$tpl->set( '{description}', "<h1>".$static_descr."</h1>" );
			}
